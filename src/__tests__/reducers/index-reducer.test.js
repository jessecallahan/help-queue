...
describe("rootReducer", () => {
  ...
  test('Check that intitial state of editingReducer matches root reducer', () => {
  ...
  test('Check that ADD_TICKET action works for ticketListReducer and root reducer', () => {
    ...
    expect(store.getState().masterTicketList).toEqual(ticketListReducer(undefined, action1));
  });
  ...
  test('Check that TOGGLE_EDITING action works for editingReducer and root reducer', () => {
    const action = {
      type: 'TOGGLE_EDITING'
    }
    store.dispatch(action);
